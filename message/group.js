...
module.exports = welcome = async (ikyy, anu) => {
		...
		try {
            ...
            buff = await getBuffer(`http://hadi-api.herokuapp.com/api/card/welcome?nama=${num.split('@')[0]}&descriminator=${memeg}&memcount=${memeg}&gcname=${encodeURI(mdata.subject)}&pp=${pp_user}&bg=${imgibb} ${encodeURI(mdata.subject)}`)
        buttons = [
      ...
      if (anu.action == "remove" && !mem.includes(ikyy.user.jid)) {
       ...
       buff = await getBuffer(`https://api.lolhuman.xyz/api/base/welcome?apikey=IkyAds&img1=${pp_user}&img2=${pp_grup}&background=https://telegra.ph/file/559d40a73f54e257b0b2e.jpg&username=${encodeURI(anu_user)}&member=${memeg}&groupname= ${encodeURI(mdata.subject)}`)
